...
const AdminTerms = () => {
  ...
  const fetchTerms = async () => {
    const res = await axios.get(`${API_BASE_URL}/api/shippings/all`);
    setTerms(res.data);
  };
  ...
  const handleSubmit = async () => {
    for (const sec of sections) {
      if (sec.heading) {
        await axios.post(`${API_BASE_URL}/api/shippings/add`, {
          heading: sec.heading,
          content: sec.content,
  ...
  const handleUpdate = async () => {
    ...
    await axios.put(`${API_BASE_URL}/api/shippings/update/${section.id}`, {
  ...
  const handleDelete = async (id) => {
    await axios.delete(`${API_BASE_URL}/api/shippings/delete/${id}`);
    fetchTerms();
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      ...
      <div className="space-y-6 container shadow-lg p-6 bg-gray-50 rounded-lg">
        <h4 className="text-lg font-semibold mb-4 bg-black text-white p-3 rounded-lg">Current Shipping Policy</h4>
        {terms.map((term, index) => (
          <div key={term._id} className="bg-gray-100 p-4 border rounded-lg shadow-sm">
